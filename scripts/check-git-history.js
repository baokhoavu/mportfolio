...
async function checkGitHistory() {
  try {
    ...
    for (const hash of commitHashes) { // Check all commits
      try {
        ...
        for (const file of files) {
          if (file && (file.endsWith('.ts') || file.endsWith('.tsx') || file.endsWith('.js') || file.endsWith('.jsx') || file.endsWith('.env'))) {
            const content = await execPromise(`git show ${hash}:${file}`);
            patterns.forEach(pattern => {
              const matches = content.match(new RegExp(pattern.regex, 'g'));
              if (matches) {
                const realMatches = matches.filter(match => {
                  return !content.includes('process.env') && !content.includes('//') && !content.includes('/*');
                });
    ...
    if (!issuesFound) {
      ...
      console.log('❌ Sensitive data detected in git history. Consider using git filter-branch or BFG to remove it.');
    }
