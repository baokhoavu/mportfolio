...
function checkForHardcodedValues() {
    ...
    patterns.forEach((pattern) => {
      ...
      if (matches) {
        ...
        const realMatches = matches.filter((match) => {
          ...
          if (context.includes("process.env") || context.includes("//") || context.includes("/*"))
            return false;
          // Skip if in import statements or CSS
          if (
            context.includes("import") ||
            context.includes(".css") ||
            context.includes("tokens.css")
          )
            return false;
          // Skip variable names
          if (
            /\b(password|token|apiKey|secret|key)\b/i.test(context) &&
            !/\b(tokens?|spacing|color)/i.test(context)
          )
            return false;
          // Skip safe URLs
  ...
  if (!issuesFound) {
    ...
    console.log(
      "❌ Hardcoded sensitive values detected. Please move them to environment variables.",
    );
    process.exit(1);
  }
}
